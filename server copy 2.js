...
async function readCsvObjects(fp) {
  const text = await fs.promises.readFile(fp, 'utf8');
  // wykorzystujemy Twój parser z pliku
  const rows = splitCsvLines(text);
  if (!rows.length) return [];
  const header = rows[0].map(h => String(h ?? '').trim());
  const out = [];
...
function hoursFromSeconds(sec) { 
  ...
  return Math.round(n / 3600); // 0 miejsc po przecinku
}
...
app.get('/api/sprint-top', async (req, res) => {
  try {
    const from  = String(req.query.from  || '').trim();
    const to    = String(req.query.to    || '').trim();
    const minh  = Math.max(0, Number(req.query.minh || 16));   // domyślnie 16h
    const limit = Math.max(1, Number(req.query.limit || 20));  // ile rekordów na team

    if (!from || !to) return res.status(400).json({ error:'missing-params', message:'from & to required' });

    const results = [];
    for (const g of GROUPS) {
      const fp = await pickExistingCsvFor({ from, to, group: g.label });
      if (!fp) continue; // pomijamy team bez CSV

      const rows = await readCsvObjects(fp);

      // agregacja po issue_key
      const agg = new Map(); // key -> { seconds, summary, status, key }
      for (const r of rows) {
        const issueKey = String(r.issue_key || r['issue_key'] || '').trim();
        if (!issueKey) continue;
        // wyklucz projekt WORKLOG
      ...
      const items = Array.from(agg.values())
        ...
        .map(x => ({
          ...
          // link do Jiry przez istniejący redirect
    ...
    res.json({
      from, to,
      teams_considered: GROUPS.length,
